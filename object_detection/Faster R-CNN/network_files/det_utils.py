...
class BoxCoder(object):
    ...
    def decode(self, rel_codes, anchors):
        ...
        # anchor 的坐标形式 从(xmin, ymin, xmax, ymax) 转换为 (ctr_x, ctr_y, widths, heights)
        widths = anchors[:, 2] - anchors[:, 0]  # anchor/proposal宽度
        heights = anchors[:, 3] - anchors[:, 1]  # anchor/proposal高度
        ctr_x = anchors[:, 0] + 0.5 * widths  # anchor/proposal中心x坐标
        ctr_y = anchors[:, 1] + 0.5 * heights  # anchor/proposal中心y坐标

        dx, dy, dw, dh = rel_codes.split([1, 1, 1, 1], dim=1) # 预测anchors/proposals的中心坐标x回归参数
        ...
        # 将预测出的 bounding box 坐标形式 由(ctr_x, ctr_y, widths, heights) 转换为 (xmin、ymin、xmax、ymax)
