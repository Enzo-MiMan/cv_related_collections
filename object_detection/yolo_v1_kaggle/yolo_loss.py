...
class yoloLoss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        # 坐标误差，xy误差 + wh误差
        loc_loss = F.mse_loss(box_pred_response[:, :2], box_target_response[:, :2], reduction='mean') \
                   + F.mse_loss(torch.sqrt(box_pred_response[:, 2:4]),torch.sqrt(box_target_response[:, 2:4]), reduction='mean')

        # 包含object的grid cell， 计算 object 置信度误差
        contain_loss = F.mse_loss(box_pred_response[:, 4], box_target_response_iou[:, 4], reduction='mean')

        # 负责检测的grid cell 中不负责检测的bboox，计算 object 置信度误差
        box_pred_not_response = box_pred[coo_not_response_mask].view(-1, 5)
        box_target_not_response = box_target[coo_not_response_mask].view(-1, 5)
        box_target_not_response[:, 4] = 0
        not_contain_loss = F.mse_loss(box_pred_not_response[:, 4], box_target_not_response[:, 4], reduction='mean')

        # 3.class loss
        class_loss = F.mse_loss(class_pred, class_target, reduction='mean')

        return self.l_coord * loc_loss + 2 * contain_loss + not_contain_loss + self.l_noobj * nooobj_loss + class_loss
