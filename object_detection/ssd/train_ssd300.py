...
def main(parser_data):
    ...
    if os.path.exists(VOC_root) is False:
        raise FileNotFoundError("PASCAL_VOC2012 dose not in path:'{}'.".format(VOC_root))

    # PASCAL_VOC2012 -> ImageSets -> Main -> train.txt
    train_dataset = VOCDataSet(VOC_root, data_transform['train'], train_set='train.txt')   # len(train_dataset)=5717
    ...
    train_data_loader = torch.utils.data.DataLoader(train_dataset,
                                                    ...
                                                    drop_last=drop_last)   # len(train_data_loader)=1429

    # VOCdevkit -> VOC2012 -> ImageSets -> Main -> val.txt
    val_dataset = VOCDataSet(VOC_root, data_transform['val'], train_set='val.txt')   # len(val_dataset)=5823
    val_data_loader = torch.utils.data.DataLoader(val_dataset,
                                                  batch_size=batch_size,
                                                  shuffle=False,
                                                  num_workers=nw,
                                                  collate_fn=train_dataset.collate_fn)   # len(val_data_loader)=1456
...
if __name__ == '__main__':
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default='./PASCAL_VOC2012', help='dataset')
    # 文件保存地址
